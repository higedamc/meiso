...
fn create_unsigned_event(
    ...
    let tags_value: Vec<serde_json::Value> = tag_rows.into_iter().map(|r| json!(r)).collect();
...
pub fn add_todo_to_mls_group(
    ...
    rt.block_on(async {
        ...
        let unsigned_event =
            create_unsigned_event(&nostr_id, &todo_json, &action, &todo_id, &group_id)?;
...
pub fn decrypt_todo_from_mls_group(
    ...
    rt.block_on(async {
        ...
        let todo_content = event["content"].as_str().unwrap_or("{}");
