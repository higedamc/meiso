...
class SyncGroupInvitationsParams {
  const SyncGroupInvitationsParams({
    ...
    this.deletedGroupIds = const <String>{},
  });
...
class SyncGroupInvitationsUseCase
  ...
  Future<Either<Failure, List<GroupInvitation>>> call(
    ...
    try {
      AppLogger.info(
        '📥 [SyncGroupInvitationsUseCase] Syncing group invitations...',
      );
      ...
      return invitationsResult.fold(
        (failure) {
          AppLogger.error(
            '❌ [SyncGroupInvitationsUseCase] Failed to sync: ${failure.message}',
          );
          return Left(failure);
        },
        (invitations) async {
          AppLogger.info(
            '✅ [SyncGroupInvitationsUseCase] Found ${invitations.length} pending invitations',
          );
          ...
          for (final invitation in invitations) {
            if (params.deletedGroupIds.contains(invitation.groupId)) {
              AppLogger.info(
                'ℹ️ [SyncGroupInvitationsUseCase] Skip invitation (locally deleted): ${invitation.groupName}',
              );
              continue;
            }
            ...
            final alreadyAccepted = existingGroup.fold(
              (_) => false,
              (group) => group != null,
            );
            if (alreadyAccepted) {
            ...
            final saveResult = await _repository.saveInvitationToLocal(
              invitation,
              blockedGroupIds: params.deletedGroupIds,
            );
          ...
          AppLogger.info(
            '✅ [SyncGroupInvitationsUseCase] Synced ${invitations.length} group invitations',
          );
          return Right(invitations);
        },
      );
