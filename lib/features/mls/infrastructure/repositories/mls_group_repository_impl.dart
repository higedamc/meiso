...
class MlsGroupRepositoryImpl implements MlsGroupRepository {
  ...
  Future<Either<Failure, GroupInvitation?>> loadInvitationFromLocal({
    ...
    try {
      final invitation = await _localDataSource.loadInvitation(
        groupId: groupId,
      );
  ...
  @override
  Future<Either<Failure, List<GroupInvitation>>>
  loadAllInvitationsFromLocal() async {
    try {
  ...
  @override
  Future<Either<Failure, void>> saveInvitationToLocal(
    GroupInvitation invitation, {
    Set<String> blockedGroupIds = const <String>{},
  }) async {
    try {
      await _localDataSource.saveInvitation(
        invitation,
  ...
  Future<Either<Failure, String>> createMlsGroup({
    ...
    try {
      ...
      AppLogger.info(
        '[MlsGroupRepo] MLS group created (Welcome: ${welcomeMsgBytes.length} bytes)',
      );
      ...
      if (welcomeMsgBytes.isEmpty) {
        AppLogger.warning(
          '⚠️ [MlsGroupRepo] Welcome Message is empty (0 bytes)',
        );
        AppLogger.warning(
          '⚠️ [MlsGroupRepo] This indicates a 1-person group (testing only)',
        );
        AppLogger.warning(
          '⚠️ [MlsGroupRepo] Production group lists require at least 2 people (self + 1 other member)',
        );
      ...
      return Left(
        MlsCryptoFailure.mlsDbInitFailed(
          'MLSグループの作成に失敗しました: $e',
        ),
      );
    }
  ...
  Future<Either<Failure, String>> sendGroupInvitation({
    ...
    try {
      AppLogger.info(
        '[MlsGroupRepo] Sending invitation to: ${recipientNpub.substring(0, 20)}...',
      );
      ...
      return Left(
        MlsNetworkFailure.networkError(
          '招待の送信に失敗しました: $e',
        ),
      );
    }
  ...
  Future<Either<Failure, List<GroupInvitation>>> syncGroupInvitations({
    ...
    try {
      ...
      AppLogger.info(
        '[MlsGroupRepo] Found ${invitationsData.length} pending invitations',
      );
      ...
      AppLogger.info(
        '[MlsGroupRepo] Parsed ${invitations.length} invitations successfully',
      );
  ...
  Future<Either<Failure, MlsGroup>> acceptGroupInvitation({
    ...
    try {
      ...
      AppLogger.debug(
        '[MlsGroupRepo] Welcome Message validation: base64Len=${welcomeMessage.length}',
      );
      ...
      AppLogger.debug(
        '[MlsGroupRepo] Welcome decoded bytes=${welcomeMsgBytes.length}',
      );
      if (welcomeMsgBytes.isEmpty) {
        return const Left(
          InvitationFailure(
